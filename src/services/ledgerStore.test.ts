...
describe("LedgerService", () => {
  ...
  describe("REGRESSION [V5.88.0]: first-boot seed generation must not lose data to the SyncHelper init lock", () => {
    // 真实发生过的数据丢失事故：首次启动服务器无台账数据时降级生成种子台账，如果 generateSeeds() 内部直接调用
    // SyncHelper.queueChange() 而不是用 runWhenInitialized() 包裹，在全局初始化 Promise.all 完成之前调用会被
    // 静默拦截丢弃，导致种子台账永远不会落盘。这里必须用真实的 SyncHelper（不 mock queueChange/runWhenInitialized）
    // 才能复现和验证这个时序问题。
    it("defers the seed ledgers/ledgerItems replaceAll ops until SyncHelper.setInitialized(true) fires", async () => {
      vi.restoreAllMocks();
      vi.useFakeTimers();
      ...
      const initPromise = LedgerService.initLedgerStore();
      await vi.advanceTimersByTimeAsync(150); // 跨过 LEDGER_API_LATENCY，触发降级生成种子台账
      await initPromise;
      ...
      const body = JSON.parse(saveCallsAfterInit[0][1]!.body as string);
      expect(body.ops).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ entity: "ledger", op: "replaceAll" }),
          expect.objectContaining({ entity: "ledgerItem", op: "replaceAll" })
        ])
      );
