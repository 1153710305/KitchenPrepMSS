...
export class LedgerService {
  ...
  public static async updateDailyRecord(
    ...
    if (fields.inQuantity !== undefined || fields.inPrice !== undefined) {
      ...
      const category = RawMaterialsDictService.getCategoryForMaterial(item.name) || "VEGETABLE";
  ...
  public static async updateDailyRecordsBatch(
    ...
    for (const updatedItem of updatedItems) {
      ...
      if (mergedRecord && (updates[updatedItem.id]?.inQuantity !== undefined || updates[updatedItem.id]?.inPrice !== undefined)) {
        const inQty = mergedRecord.inQuantity ?? 0;
        const inPr = mergedRecord.inPrice ?? 0;
        const category = RawMaterialsDictService.getCategoryForMaterial(updatedItem.name) || "VEGETABLE";
