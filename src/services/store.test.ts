...
describe("PrepReportService", () => {
  ...
  describe("REGRESSION [V5.88.0]: first-boot seed generation must not lose data to the SyncHelper init lock", () => {
    // 真实发生过的数据丢失事故：首次启动服务器无数据时降级生成种子人群/大类/报表，如果直接调用
    // SyncHelper.queueChange() 而不是用 runWhenInitialized() 包裹，在全局初始化 Promise.all 完成之前调用
    // 会被静默拦截丢弃，导致这批种子数据永远不会落盘。这里必须用真实的 SyncHelper
    // （不 mock queueChange/runWhenInitialized）才能复现和验证这个时序问题。
    it("defers the seed activeGroup/activeCategory/report replaceAll ops until SyncHelper.setInitialized(true) fires", async () => {
      vi.restoreAllMocks();
      vi.useFakeTimers();
      const fetchSpy = vi.fn((url: string, _options?: RequestInit) => {
        if (url === "/api/storage/load") {
      ...
      const initPromise = PrepReportService.initStore();
      await vi.advanceTimersByTimeAsync(200); // 跨过 MOCK_API_LATENCY，触发降级生成种子人群/大类/报表
      await initPromise;
      ...
      expect(saveCallsAfterInit.length).toBeGreaterThan(0);
      const allOps = saveCallsAfterInit.flatMap(([, options]) => JSON.parse(options!.body as string).ops);
      expect(allOps).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ entity: "activeGroup", op: "replaceAll" }),
          expect.objectContaining({ entity: "activeCategory", op: "replaceAll" }),
          expect.objectContaining({ entity: "report", op: "replaceAll" })
        ])
      );
